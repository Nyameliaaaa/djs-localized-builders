...
export interface PermsV2Mixin<_T extends SlashCommandBuilder | ContextMenuCommandBuilder> extends BuilderMixin<_T> {}

@mix(BuilderMixin)
export class PermsV2Mixin<_T extends SlashCommandBuilder | ContextMenuCommandBuilder> {
...
export class SharedOptionsMixin<_T extends SlashCommandBuilder | SlashCommandSubcommandBuilder> {
	 ...
	 * @internal
	 */
	protected isString(input: unknown): input is string {
		return typeof input === 'string';
	}

	private isFunction<T>(input: unknown): input is FuncAsInput<T> {
		return typeof input === 'function' && !this.isBuilder(input);
	}

	private isBuilder<T>(input: unknown): input is T {
	...
	private handleCall<T extends OptionMixin<OptionResolvable>>(
		...
		OptionType: new (keySegment?: string) => T
	) {
