...
export class StringSelectMenuOptionBuilder {
	...
	private descArgs: Record<string, unknown> = {};
	...
	setDescription(descriptionOrArgs?: string | Record<string, unknown>, args: ArgsWithRawOrKeyedParam = {}) {
		/** ====================================================================================
		 *   HOW VALID STATES ARE HANDLED
		 * =====================================================================================
		 * * setDescription()
		...
		if (isValidationEnabled()) {
			...
			if (typeof descriptionOrArgs === 'string' && args.localized && args.raw) {
				throw new TypeError('Cannot be raw and localized at once.', { cause: descriptionOrArgs });
			}
		}
